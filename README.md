# 🏗 Scaffold-ETH - 🎟 Hashlips Simple NFT Example
> Build, mint unique nfts, and send around your own ERC721 Collection!

# 🏃‍♀️ Quick Start
Required: [Node](https://nodejs.org/dist/latest-v12.x/) plus [Yarn](https://classic.yarnpkg.com/en/docs/install/#mac-stable) and [Git](https://git-scm.com/downloads)


All hashlips code in these repos was created and explained by HashLips on the main YouTube channel.
...
```
git clone https://github.com/blahkheart/scaffold-eth.git hashlips-simple-nft
```
```
cd hashlips-simple-nft
git checkout hashlips-simple-nft
yarn install
yarn start
```
...
> ✏️ Edit the mint script mint.js in packages/hardhat/scripts and update the owner to your frontend address (wallet address in the top right or localhost:3000).

```
TIP: Remeber to go to Debug contracts tab on the frontend and 'Reveal' your collectibles to be able to view your NFTs when you mint,
else the 'NotRevealedURI' is set as default.

```

## Hashlip art engine Usage ℹ️
...
When you are ready, run the following command and your outputted art will be in the `src/output/images` directory and the json in the `output/json` directory:
...
The program will output all the images in the `output/images` directory along with the metadata files in the `output/json` directory. Each collection will have a `_metadata.json` file that consists of all the metadata in the collection inside the `output/json` directory. The `output/json` folder also will contain all the single json files that represent each image file. The single json file of a image will look something like this:
